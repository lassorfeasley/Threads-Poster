...
from .db import session_scope, sync_channels_from_config
...
def poll_channel(session, channel: Channel, keywords: list[str], settings,
    ...
    are unique in the store. Returns count stored.
    """
    ...
    for up in uploads:
        ...
        stored += 1
        log.info("Candidate: [%s] %s (score=%s)", channel.call_sign, up.title, candidate.relevance_score)
...
def run_monitor_once(lookback_days: int | None = None) -> dict:
    ...
    with session_scope() as session:
        ...
        for channel in channels:
            ...
            session.commit()
    log.info("Monitor pass done: %d channels checked, %d new candidates", checked, total_stored)
